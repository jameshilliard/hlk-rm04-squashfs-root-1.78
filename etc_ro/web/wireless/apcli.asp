...
function checkssid()
{
	var count=0; 
	var i,j;
	var ssid=document.wireless_apcli.apcli_ssid.value;
	if(ssid != '')
		for (i=2; i < window.site_table.rows.length; i++) { 
//			for (j=0; j < window.site_table.rows[i].cells.length; j++) { 
//				alert(window.site_table.rows[i].cells[j].innerHTML); 
//			} 
			if(window.site_table.rows[i].cells[1].innerHTML == ssid){
				document.wireless_apcli.ap_channel.value=window.site_table.rows[i].cells[0].innerHTML;
				break;
			}
		}
}
...
<table width = "540" border = "0" cellpadding = "2" cellspacing = "1">
  <tr align="center">
    <td>
      <input type=submit style="{width:120px;}" value="Apply" onClick="checkssid()"> &nbsp; &nbsp;
...
<table  id="site_table" width="600" border="1" cellpadding="2" cellspacing="1">
  <tr> 
