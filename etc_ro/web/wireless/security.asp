...
<link rel="stylesheet" href="/style/normal_ws.css" type="text/css"><title>HLK-RM04 Wireless Security Settings</title>
...
function securityMode(c_f)
{
	...
	if (security_mode == "OPEN" || security_mode == "SHARED" ||security_mode == "WEPAUTO")
	{
		...
		document.getElementById("div_wpa").style.display = style_display_on();

		document.getElementById("div_wpa_algorithms").style.visibility = "visible";
		if (window.ActiveXObject) { // IE
			document.getElementById("div_wpa_algorithms").style.display = "block";
		}
		else if (window.XMLHttpRequest) { // Mozilla, Safari,...
			document.getElementById("div_wpa_algorithms").style.display = "table-row";
		}
		...
		if (window.ActiveXObject) { // IE
			document.getElementById("wpa_passphrase").style.display = "block";
		}
		else if (window.XMLHttpRequest) { // Mozilla, Safari,...
			document.getElementById("wpa_passphrase").style.display = "table-row";
		}
		...
		if (window.ActiveXObject) { // IE
			document.getElementById("wpa_key_renewal_interval").style.display = "block";
		}
		else if (window.XMLHttpRequest) { // Mozilla, Safari,...
			document.getElementById("wpa_key_renewal_interval").style.display = "table-row";
		}
	...
	}else if (security_mode == "WPA" || security_mode == "WPA2" || security_mode == "WPA1WPA2") //wpa enterprise
	{
		...
		if(security_mode == "WPA2"){
			...
			if (window.ActiveXObject) { // IE
				document.getElementById("wpa_preAuthentication").style.display = "block";
			}
			else if (window.XMLHttpRequest) { // Mozilla, Safari,...
				document.getElementById("wpa_preAuthentication").style.display = "table-row";
			}
			...
			if (window.ActiveXObject) { // IE
				document.getElementById("wpa_PMK_Cache_Period").style.display = "block";
			}
			else if (window.XMLHttpRequest) { // Mozilla, Safari,...
				document.getElementById("wpa_PMK_Cache_Period").style.display = "table-row";
			}
...
function ShowAP(MBSSID)
{
	...
	document.getElementById("AccessPolicy_"+MBSSID).style.display = style_display_on();
}
