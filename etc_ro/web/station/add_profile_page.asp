...
<title>HLK-RM04 Wireless Station Add Profile</title>
...
//	document.profile_page.channel.disabled = true;
	...
	//alert("add profile: "+document.profile_page.Ssid.value);
	networkTypeChange();
	RTSThresholdChange();
...
function checkData()
{
	...
	if (document.profile_page.fragment_threshold.checked)
	{
		if (isNaN(document.profile_page.fragment_thresholdvalue.value) ||
		    (document.profile_page.fragment_thresholdvalue.value < 256 ||
		     document.profile_page.fragment_thresholdvalue.value > 2346))
	{
			alert("Invalid the length of RTS Threshold");
		return false;
	}
	}
	if (document.profile_page.rts_threshold.checked)
	{
		if (isNaN(document.profile_page.rts_thresholdvalue.value) ||
	    	    (document.profile_page.rts_thresholdvalue.value < 1 ||
	     	     document.profile_page.rts_thresholdvalue.value > 2347))
		{
			alert("Invalid the length of Fragmentation Threshold");
			return false;
		}
	}
...
function check_Wep(securitymode)
{
	...
	if (keyvalue.length == 0 && ((securitymode == 0 && document.profile_page.security_encryp_mode.value == 1) ||
...
function submit_apply()
{
	...
	//	document.getElementById("profile_page").submit();
		//var formx = document.getElementById("profile_page");
		//formx.submit();
		opener.location.reload();
//		window.close();
		close_w();
...
<table width="540" border="1" cellspacing="1" cellpadding="3" vspace="2" hspace="2" bordercolor="#9BABBD">
  ...
  <tr>
    ...
    <td>
      ...
      <input type=text name=rts_thresholdvalue value=2347> (range 1 - 2347, default 2347)
    </td>
  </tr>
  <tr>
    ...
    <td>
      ...
      <input type=text name=fragment_thresholdvalue value=2346> (range 256 - 2346, default 2346)
    </td>
  </tr>
</table>
...
<table id="div_wep" name="div_wep" width="540" border="1" cellspacing="1" cellpadding="3" vspace="2" hspace="2" bordercolor="#9BABBD">
  ...
  <tr> 
    ...
    <td>
      <select name="wep_default_key" size="1" onChange="wep_default_key_switch()">
...
<table id="div_8021x" name="div_8021x" width="540" border="1" cellspacing="1" cellpadding="3" vspace="2" hspace="2" bordercolor="#9BABBD">
  ...
  <tr id="div_client_cert_path" name="div_client_cert_path">
    ...
    <td>
      ...
      <input type="button" value="Install" name="ca_cl_cert_upload" id="ca_cl_cert_upload" onClick="open_cert_upload('client')">
    </td>
  </tr>
  </tr>
