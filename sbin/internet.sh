...
config-powersave.sh ethernet 1 0;config-powersave.sh ethernet 1 1;config-powersave.sh ethernet 1 2;
config-powersave.sh ethernet 1 3;config-powersave.sh ethernet 1 4;
...
AT_netmode=`nvram_get 2860 AT_netmode`

if [ "$AT_netmode" = "0" ]; then
	if [ "$radio_off1" != "1" ]; then
		iwpriv ra0 set RadioOn=1
	fi
	config-powersave.sh ethernet 0 3;
	config-powersave.sh ethernet 0 4;
fi

if [ "$AT_netmode" = "1" ]; then
	config-powersave.sh ethernet 0 4;
fi

if [ "$AT_netmode" = "2" ]; then
	if [ "$radio_off1" != "1" ]; then
		iwpriv ra0 set RadioOn=1
	fi
fi
if [ "$AT_netmode" = "3" ]; then
	if [ "$radio_off1" != "1" ]; then
		iwpriv ra0 set RadioOn=1
	fi
fi
